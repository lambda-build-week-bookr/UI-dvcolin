...
toggleNav.addEventListener("click", function() {
  ...
  overlay.classList.add("darken-overlay");
});
...
class aboutButton {
  constructor(element) {
    ...
    if (this.jobData === "all") {
      ...
      this.members = document.querySelectorAll(
        `.about__member[data-job = '${this.jobData}']`
      );
    }

    this.members = Array.from(this.members).map(
      member => new TeamMember(member)
    );
